...
// Return true when the latest stable state of the inside exit sensor is BLOCK.
bool ldr_exit_sensor_is_blocked(void);

// Return true when both sensors are currently in their stable BLOCK state.
bool ldr_both_sensors_are_blocked(void);

// Preserve a closed-door simultaneous request while the FSM waits for one
// side to step back. This does not assign ownership to either direction.
void ldr_start_simultaneous_hold(void);

// Observe an existing LDR2 request briefly before assigning EXIT ownership.
