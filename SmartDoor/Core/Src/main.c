...
static HAL_StatusTypeDef PN532_SAMConfig(void);
static int      PN532_ReadPassiveTargetUID(uint8_t *uid, uint8_t *uidLen);
static void     PN532_PrintUID(const uint8_t *uid, uint8_t uidLen);
...
    {
        printf("[ERROR] SAMConfig transmit failed.\r\n");
        return HAL_ERROR;
    }

    if (PN532_WaitReady(PN532_READY_RETRIES) != HAL_OK)
    {
        printf("[ERROR] No ACK from PN532 after SAMConfig.\r\n");
        return HAL_TIMEOUT;
    }

    if (PN532_ReadResponse(buf, 7) != HAL_OK)
    {
        printf("[ERROR] Failed to read ACK frame.\r\n");
        return HAL_ERROR;
    }
...
/**
  * @brief  寻找一张 ISO14443A 卡片并取出其 UID
  */
static int PN532_ReadPassiveTargetUID(uint8_t *uid, uint8_t *uidLen)
{
...
  {
      printf("[FATAL] PN532 init failed. Halting.\r\n");
      while (1)
      {
          HAL_Delay(1000);
      }
...
  printf("PN532 initialised. Place a card near the antenna...\r\n");
...
      if (PN532_ReadPassiveTargetUID(uid, &uidLen))
      {
          PN532_PrintUID(uid, uidLen);
          HAL_Delay(1000);   /* 读到卡后暂停，避免同一张卡刷屏 */
      }

      HAL_Delay(100);        /* 主循环空闲间隔 */
